...
// options: string[] OR {value, label}[] — backward compatible
function SearchableSelect({ options, value, onChange, placeholder }) {
  ...
  // label to show in the trigger for the currently selected value
  const selectedLabel = (() => {
    if (!value) return null
    const match = (options || []).find(o => optVal(o) === value)
  ...
  const filtered = (options || []).filter(o =>
    !search.trim() || optLabel(o).toLowerCase().includes(search.toLowerCase())
  )
  ...
  return (
    <div ref={ref} style={{ position:'relative' }}>
      ...
      <div
        onClick={toggleOpen}
        style={{ ...iS, cursor:'pointer', display:'flex', justifyContent:'space-between',
          alignItems:'center', background:'#fff', userSelect:'none',
          color: value ? '#111' : '#9ca3af' }}>
        <span style={{ overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', flex:1 }}>
          {selectedLabel || placeholder || '— Select —'}
        </span>
        <span style={{ marginLeft:8, fontSize:10, color:'#9ca3af', flexShrink:0 }}>
          {open ? '▲' : '▼'}
        </span>
      </div>

      {/* Dropdown panel — position:fixed so modal overflow doesn't clip it */}
      {open && dropRect && (
        <div style={{
          position:'fixed', top: dropRect.bottom + 3, left: dropRect.left, width: dropRect.width,
          ...
          <div
            ...
            style={{ padding:'7px 12px', fontSize:12, color:'#9ca3af',
              cursor:'pointer', borderBottom:'1px solid #f9fafb', flexShrink:0 }}
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    color: value === v ? '#0C447C' : '#111',
                    fontWeight: value === v ? 600 : 400,
                    borderBottom:'1px solid #f9fafb',
                  }}
                  onMouseOver={e => { if(value!==v) e.currentTarget.style.background='#f8fafc' }}
                  onMouseOut={e => { if(value!==v) e.currentTarget.style.background='#fff' }}>
                  {l}
                </div>
              )
            })}
